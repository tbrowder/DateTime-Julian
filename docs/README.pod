...
Provides a DateTime::Julian class (a subclass of Raku's class B<DateTime>) 
that is instantiated by either a Julian Date (JD) or a
...
is instantiated from a I<Julian Date> or a I<Modified Julian Date>.
...
If both arguments are entered, the I<Julian Date> is used. If neither is
entered, an exception is thrown.

Note that currently none of the ordinary DateTime I<new> methods can
be used for instantiation, but that could be done if someone
can justify it.
...
=head2 Class DateTime::Julian subroutines
...
Converts a date in the Julian calendar to the
equivalent date in the Gregorian calendar.
...
Converts a date in the Gregorian calendar to the
